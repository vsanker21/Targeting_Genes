#!/usr/bin/env python3
"""
Outline M1: optional reference + GDC open STAR staging path presence (see data_sources.yaml).

Config: config/m1_reference_gdc_outline_inputs.yaml — m1_reference_gdc_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m1_reference_gdc_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m1_reference_gdc_path_checks") or {}
    if not block.get("enabled", True):
        print("m1_reference_gdc_path_checks disabled")
        return 0
    ...
    out_doc = {
        ...
        "outline_module": 1,
        "artifact_kind": "m1_reference_gdc_paths_status",
        "groups": groups_out,
        "note": "Presence only; no GDC API or reference download jobs from this JSON.",
    }
    out_rel = str(block.get("output_json", "results/module3/m1_reference_gdc_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module3/m1_reference_gdc_paths_status.flag"))
