#!/usr/bin/env python3
"""
Outline M2.1: optional GDC/GTEx STAR count mirror path presence (paired bulk DEA staging).

Config: config/m2_1_star_pairing_outline_inputs.yaml — m2_1_star_pairing_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m2_1_star_pairing_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8"))
    block = doc.get("m2_1_star_pairing_path_checks") or {}
    if not block.get("enabled", True):
        print("m2_1_star_pairing_path_checks disabled")
        return 0
    ...
    out_doc = {
        ...
        "artifact_kind": "m2_1_star_pairing_paths_status",
        "groups": groups_out,
        "note": "Presence only; no GDC API or STAR realignment jobs from this JSON.",
    }
    out_rel = str(block.get("output_json", "results/module3/m2_1_star_pairing_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module3/m2_1_star_pairing_paths_status.flag"))
