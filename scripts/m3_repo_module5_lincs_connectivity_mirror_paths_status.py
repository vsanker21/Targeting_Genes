#!/usr/bin/env python3
"""
Outline M3: presence of Module 5 m5_lincs_connectivity_mirror outline artifacts under results/module5.

Config: config/m3_repo_module5_lincs_connectivity_mirror_outline_inputs.yaml —
m3_repo_module5_lincs_connectivity_mirror_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m3_repo_module5_lincs_connectivity_mirror_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m3_repo_module5_lincs_connectivity_mirror_path_checks") or {}
    if not block.get("enabled", True):
        print("m3_repo_module5_lincs_connectivity_mirror_path_checks disabled")
        return 0
    ...
    note = (
        "M3 manifest slice for m5_lincs_connectivity_mirror_paths_status and integration_stub under results/module5; "
        "data_root/lincs connectivity mirrors remain in m5_lincs_connectivity_mirror_paths_status.json groups."
    )
    out_doc = {
    ...
    d_out = "results/module3/m3_repo_module5_lincs_connectivity_mirror_paths_status.json"
    d_flag = "results/module3/m3_repo_module5_lincs_connectivity_mirror_paths_status.flag"
