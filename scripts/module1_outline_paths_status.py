#!/usr/bin/env python3
"""
Outline M1: record grouped optional path presence under data_root (no downloads).

Config: config/m1_outline_inputs.yaml — m1_outline_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m1_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8"))
    block = doc.get("m1_outline_path_checks") or {}
    if not block.get("enabled", True):
        print("m1_outline_path_checks disabled")
        return 0
    ...
    out = {
        ...
        "outline_module": 1,
        "groups": groups_out,
        "note": "Presence only; outline M1 ingestion and FASTQ reprocessing are not executed from this JSON.",
    }
    out_rel = str(block.get("output_json", "results/module3/m1_outline_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module3/m1_outline_paths_status.flag"))
