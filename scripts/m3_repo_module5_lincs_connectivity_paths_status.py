#!/usr/bin/env python3
"""
Outline M3: presence of Module 5 LINCS signature and connectivity artifacts under results/module5.

Config: config/m3_repo_module5_lincs_connectivity_outline_inputs.yaml — m3_repo_module5_lincs_connectivity_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m3_repo_module5_lincs_connectivity_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m3_repo_module5_lincs_connectivity_path_checks") or {}
    if not block.get("enabled", True):
        print("m3_repo_module5_lincs_connectivity_path_checks disabled")
        return 0
    ...
    out_doc = {
        ...
        "outline_module": 3,
        "groups": groups_out,
        "note": "Bulk DEA inputs from module3; Entrez signatures for LINCS / cmapPy planning under results/module5.",
    }
    out_rel = str(
        block.get("output_json", "results/module3/m3_repo_module5_lincs_connectivity_paths_status.json")
    )
    flag_rel = str(
        block.get("done_flag", "results/module3/m3_repo_module5_lincs_connectivity_paths_status.flag")
    )
