#!/usr/bin/env python3
"""
Outline M7 optional external GTS-like score mirror paths under data_root.

Config: config/m7_gts_external_score_mirror_outline_inputs.yaml (m7_gts_external_score_mirror_path_checks).
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m7_gts_external_score_mirror_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m7_gts_external_score_mirror_path_checks") or {}
    if not block.get("enabled", True):
        print("m7_gts_external_score_mirror_path_checks disabled")
        return 0
    ...
    out_doc = {
        ...
        "outline_section": "M7",
        "artifact_kind": "m7_gts_external_score_mirror_paths_status",
        "groups": groups_out,
        "note": "Presence only; in-repo GTS stub remains export_gts_candidate_table.py and m7_gts_candidate_stub.",
    }
    out_rel = str(block.get("output_json", "results/module7/m7_gts_external_score_mirror_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module7/m7_gts_external_score_mirror_paths_status.flag"))
