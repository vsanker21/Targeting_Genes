#!/usr/bin/env python3
"""
Outline M3: presence of module3_sc_workflow + scrna_spatial_integration_stub artifacts under results/module3.

Config: config/m3_repo_scrna_spatial_outline_inputs.yaml — m3_repo_scrna_spatial_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m3_repo_scrna_spatial_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m3_repo_scrna_spatial_path_checks") or {}
    if not block.get("enabled", True):
        print("m3_repo_scrna_spatial_path_checks disabled")
        return 0
    ...
    note = (
        "M3 manifest slice for module3_sc_workflow_paths_status and scrna_spatial_integration_stub under results/module3; "
        "data_root Scanpy/Seurat/Visium trees remain in module3_inputs.yaml sc_workflow_path_checks."
    )
    out_doc = {
    ...
    d_out = "results/module3/m3_repo_scrna_spatial_paths_status.json"
    d_flag = "results/module3/m3_repo_scrna_spatial_paths_status.flag"
