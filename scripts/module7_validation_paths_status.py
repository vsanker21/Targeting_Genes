#!/usr/bin/env python3
"""
Outline M7: grouped optional validation / benchmark staging path presence under data_root.

Config: config/m7_validation_outline_inputs.yaml — m7_validation_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m7_validation_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8"))
    block = doc.get("m7_validation_path_checks") or {}
    if not block.get("enabled", True):
        print("m7_validation_path_checks disabled")
        return 0
    ...
    out = {
        ...
        "outline_module": 7,
        "groups": groups_out,
        "note": "Presence only; no AUC computation, survival modeling, or benchmark diff jobs in-repo.",
    }
    out_rel = str(block.get("output_json", "results/module7/m7_validation_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module7/m7_validation_paths_status.flag"))
