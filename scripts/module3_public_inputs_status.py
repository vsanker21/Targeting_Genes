#!/usr/bin/env python3
"""
Module 3 (outline): record availability of public scRNA, spatial, and SRA paths under data_root.

Does not fail if paths are missing (partial implementation per pipeline_outline.yaml).
Writes results/module3/module3_public_inputs_status.json for planning Scanpy/Seurat rules.

Paths are defined in config/module3_inputs.yaml.
"""
...
def main() -> int:
    ...
    out = {
        ...
        "outline_module": 3,
        "checks": rows,
        "note": "Presence only; downstream Module 3 rules should consume these paths when exists=true.",
    }
    out_path = rr / "results" / "module3" / "module3_public_inputs_status.json"
    flag_path = rr / "results" / "module3" / "module3_public_inputs_status.flag"
