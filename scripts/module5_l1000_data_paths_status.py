#!/usr/bin/env python3
"""
Outline M5: optional L1000 GCTX / CLUE / cmap reference staging path presence under data_root.

Config: config/m5_l1000_data_outline_inputs.yaml — m5_l1000_data_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m5_l1000_data_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8"))
    block = doc.get("m5_l1000_data_path_checks") or {}
    if not block.get("enabled", True):
        print("m5_l1000_data_path_checks disabled")
        return 0
    ...
    out_doc = {
        ...
        "outline_module": 5,
        "groups": groups_out,
        "note": "Presence only; no GCTX parsing, cmapPy query batches, or CLUE API calls in-repo.",
    }
    out_rel = str(block.get("output_json", "results/module5/m5_l1000_data_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module5/m5_l1000_data_paths_status.flag"))
