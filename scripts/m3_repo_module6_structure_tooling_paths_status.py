#!/usr/bin/env python3
"""
Outline M3: presence of Module 6 structure tooling path-status + structure_admet stub under results/module6.

Config: config/m3_repo_module6_structure_tooling_outline_inputs.yaml —
m3_repo_module6_structure_tooling_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m3_repo_module6_structure_tooling_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m3_repo_module6_structure_tooling_path_checks") or {}
    if not block.get("enabled", True):
        print("m3_repo_module6_structure_tooling_path_checks disabled")
        return 0
    ...
    note = (
        "M3 manifest slice for module6_structure_tooling_paths_status and structure_admet_integration_stub "
        "under results/module6; pocket/docking roots remain in module6_inputs.yaml structure_tooling_path_checks."
    )
    out_doc = {
    ...
    d_out = "results/module3/m3_repo_module6_structure_tooling_paths_status.json"
    d_flag = "results/module3/m3_repo_module6_structure_tooling_paths_status.flag"
