#!/usr/bin/env python3
"""
Outline M4: grouped optional GRN / co-expression staging path presence under data_root.

Config: config/m4_network_outline_inputs.yaml — m4_network_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m4_network_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8"))
    block = doc.get("m4_network_path_checks") or {}
    if not block.get("enabled", True):
        print("m4_network_path_checks disabled")
        return 0
    ...
    out = {
        ...
        "outline_module": 4,
        "groups": groups_out,
        "note": "Presence only; WGCNA, SCENIC+, BioGRID metrics, and SLAYER-style pairs are not computed in-repo.",
    }
    out_rel = str(block.get("output_json", "results/module4/m4_network_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module4/m4_network_paths_status.flag"))
