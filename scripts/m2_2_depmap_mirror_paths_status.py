#!/usr/bin/env python3
"""
Outline M2.2: optional DepMap mirror path presence under data_root/depmap/.

Config: config/m2_2_depmap_mirror_outline_inputs.yaml — m2_2_depmap_mirror_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m2_2_depmap_mirror_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m2_2_depmap_mirror_path_checks") or {}
    if not block.get("enabled", True):
        print("m2_2_depmap_mirror_path_checks disabled")
        return 0
    ...
    out_doc = {
        ...
        "artifact_kind": "m2_2_depmap_mirror_paths_status",
        "groups": groups_out,
        "note": "Presence only; no DepMap portal download jobs from this JSON.",
    }
    out_rel = str(block.get("output_json", "results/module3/m2_2_depmap_mirror_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module3/m2_2_depmap_mirror_paths_status.flag"))
