#!/usr/bin/env python3
"""
Outline M5 supplement: optional sRGES score export / compound prior / pathway projection staging.

Config: config/m5_srges_output_outline_inputs.yaml — m5_srges_output_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m5_srges_output_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m5_srges_output_path_checks") or {}
    if not block.get("enabled", True):
        print("m5_srges_output_path_checks disabled")
        return 0
    ...
    out_doc = {
        ...
        "outline_module": 5,
        "groups": groups_out,
        "note": "Presence only; no sRGES, cmapPy batch scoring, or pathway projection execution in-repo.",
    }
    out_rel = str(block.get("output_json", "results/module5/m5_srges_output_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module5/m5_srges_output_paths_status.flag"))
