#!/usr/bin/env python3
"""
Outline M3: TCGA-GBM case tree, recount3 G029 cache, LINCS clue subset root under data_root (presence only).

Config: config/m3_tcga_recount_lincs_mirror_outline_inputs.yaml — m3_tcga_recount_lincs_mirror_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m3_tcga_recount_lincs_mirror_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m3_tcga_recount_lincs_mirror_path_checks") or {}
    if not block.get("enabled", True):
        print("m3_tcga_recount_lincs_mirror_path_checks disabled")
        return 0
    ...
    out_doc = {
        ...
        "outline_module": 3,
        "groups": groups_out,
        "note": "Presence only; complements m1_reference_gdc paths; no GDC, recount3, or CLUE jobs in-repo.",
    }
    out_rel = str(block.get("output_json", "results/module3/m3_tcga_recount_lincs_mirror_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module3/m3_tcga_recount_lincs_mirror_paths_status.flag"))
