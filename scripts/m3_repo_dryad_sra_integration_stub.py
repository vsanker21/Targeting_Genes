#!/usr/bin/env python3
"""
Outline M3: integration stub for m3_repo_dryad_sra manifest slice (echoes Dryad/SRA mirror + scRNA/spatial repo stubs).

Config: config/m3_repo_dryad_sra_outline_inputs.yaml — m3_repo_dryad_sra_integration_stub
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m3_repo_dryad_sra_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m3_repo_dryad_sra_integration_stub") or {}
    if not block.get("enabled", True):
        print("m3_repo_dryad_sra_integration_stub disabled")
        return 0

    dryad_stub = rr / "results" / "module3" / "m3_dryad_sra_integration_stub.json"
    scrna_repo_stub = rr / "results" / "module3" / "m3_repo_scrna_spatial_integration_stub.json"
    echo_paths = [dryad_stub, scrna_repo_stub]
    ...
    out_doc: dict[str, Any] = {
        ...
        "outline_module": 3,
        "group": "module3_dryad_sra_repo_outline_outputs",
        "echo_integration_stubs": [str(p.resolve()) for p in echo_paths],
        "note": (
            "M3 repo Dryad/SRA manifest slice: echoes m3_dryad_sra_integration_stub and "
            "m3_repo_scrna_spatial_integration_stub (scRNA/spatial repo outline)."
        ),
    }
    d_out = "results/module3/m3_repo_dryad_sra_integration_stub.json"
    d_flag = "results/module3/m3_repo_dryad_sra_integration_stub.flag"
