...
Config: config/m4_pathway_database_mirror_outline_inputs.yaml (m4_pathway_database_mirror_path_checks).
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m4_pathway_database_mirror_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m4_pathway_database_mirror_path_checks") or {}
    if not block.get("enabled", True):
        print("m4_pathway_database_mirror_path_checks disabled")
        return 0
    ...
    out_doc = {
        ...
        "outline_section": "M4",
        "artifact_kind": "m4_pathway_database_mirror_paths_status",
        "groups": groups_out,
        "note": "Presence only; use with clusterProfiler, pathview, or ReactomePA outside this repo.",
    }
    out_rel = str(
        block.get("output_json", "results/module4/m4_pathway_database_mirror_paths_status.json")
    )
    flag_rel = str(block.get("done_flag", "results/module4/m4_pathway_database_mirror_paths_status.flag"))
