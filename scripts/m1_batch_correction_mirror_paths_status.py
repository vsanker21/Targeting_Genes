#!/usr/bin/env python3
"""
Outline M1 optional batch-corrected expression mirror paths under data_root.

Config: config/m1_batch_correction_mirror_outline_inputs.yaml (m1_batch_correction_mirror_path_checks).
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m1_batch_correction_mirror_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m1_batch_correction_mirror_path_checks") or {}
    if not block.get("enabled", True):
        print("m1_batch_correction_mirror_path_checks disabled")
        return 0
    ...
    out_doc = {
        ...
        "outline_section": "M1",
        "artifact_kind": "m1_batch_correction_mirror_paths_status",
        "groups": groups_out,
        "note": "Presence only; in-repo ComBat-Seq stub uses combat_seq_tcga_gbm.yaml and results/module1/combat_seq_tcga_gbm_primary/.",
    }
    out_rel = str(block.get("output_json", "results/module3/m1_batch_correction_mirror_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module3/m1_batch_correction_mirror_paths_status.flag"))
