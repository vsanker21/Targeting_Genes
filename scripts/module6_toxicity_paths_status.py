#!/usr/bin/env python3
"""
Outline M6: optional CNS / off-tumor / organ toxicity staging path presence under data_root.

Config: config/m6_toxicity_outline_inputs.yaml — m6_toxicity_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m6_toxicity_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8"))
    block = doc.get("m6_toxicity_path_checks") or {}
    if not block.get("enabled", True):
        print("m6_toxicity_path_checks disabled")
        return 0
    ...
    out_doc = {
        ...
        "outline_module": 6,
        "groups": groups_out,
        "note": "Presence only; no toxicity scoring, BBB models, or organ panel tests in-repo.",
    }
    out_rel = str(block.get("output_json", "results/module6/m6_toxicity_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module6/m6_toxicity_paths_status.flag"))
