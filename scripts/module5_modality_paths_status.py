#!/usr/bin/env python3
"""
Outline M5: record grouped optional TMZ / CAR-T staging path presence under data_root.

Config: config/m5_modality_outline_inputs.yaml — m5_modality_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m5_modality_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8"))
    block = doc.get("m5_modality_path_checks") or {}
    if not block.get("enabled", True):
        print("m5_modality_path_checks disabled")
        return 0
    ...
    out = {
        ...
        "outline_module": 5,
        "groups": groups_out,
        "note": "Presence only; TMZ sensitization and CAR-T surface funnel computations are not run in-repo.",
    }
    out_rel = str(block.get("output_json", "results/module5/m5_modality_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module5/m5_modality_paths_status.flag"))
