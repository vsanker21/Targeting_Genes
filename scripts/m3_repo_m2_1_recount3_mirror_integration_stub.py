...
Config: config/m3_repo_m2_1_recount3_mirror_outline_inputs.yaml — m3_repo_m2_1_recount3_mirror_integration_stub
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m3_repo_m2_1_recount3_mirror_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m3_repo_m2_1_recount3_mirror_integration_stub") or {}
    if not block.get("enabled", True):
        print("m3_repo_m2_1_recount3_mirror_integration_stub disabled")
        return 0

    m3 = rr / "results" / "module3"
    echo_paths = [m3 / "m2_1_recount3_mirror_integration_stub.json", m3 / "m2_1_star_pairing_integration_stub.json"]
    ...
    out_doc: dict[str, Any] = {
        ...
        "outline_module": 2,
        "group": "module3_m2_1_recount3_mirror_repo_outline_outputs",
        "echo_integration_stubs": [str(p.resolve()) for p in echo_paths],
        "note": "M3 manifest slice for m2_1_recount3_mirror; echoes recount3 + STAR pairing stubs.",
    }
