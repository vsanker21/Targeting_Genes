#!/usr/bin/env python3
"""
Outline M1 supplement: optional Harmony / scRNA batch staging path presence under data_root.

Config: config/m1_harmony_batch_outline_inputs.yaml — m1_harmony_batch_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m1_harmony_batch_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8"))
    block = doc.get("m1_harmony_batch_path_checks") or {}
    if not block.get("enabled", True):
        print("m1_harmony_batch_path_checks disabled")
        return 0
    ...
    out = {
        ...
        "outline_module": 1,
        "artifact_kind": "m1_harmony_batch_paths_status",
        "groups": groups_out,
        "note": "Presence only; Harmony, Scanpy, Seurat, and scVI are not executed from this JSON.",
    }
    out_rel = str(block.get("output_json", "results/module3/m1_harmony_batch_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module3/m1_harmony_batch_paths_status.flag"))
