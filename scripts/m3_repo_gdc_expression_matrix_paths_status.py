#!/usr/bin/env python3
"""
Outline M3: presence of in-repo GDC STAR matrices under results/module2 (data_sources gdc repo paths).

Config: config/m3_repo_gdc_expression_matrix_outline_inputs.yaml — m3_repo_gdc_expression_matrix_path_checks
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m3_repo_gdc_expression_matrix_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m3_repo_gdc_expression_matrix_path_checks") or {}
    if not block.get("enabled", True):
        print("m3_repo_gdc_expression_matrix_path_checks disabled")
        return 0
    ...
    out_doc = {
        ...
        "outline_module": 3,
        "groups": groups_out,
        "note": "Built outputs in clone; staging of raw STAR TSVs remains under data_root (m1_reference_gdc).",
    }
    out_rel = str(block.get("output_json", "results/module3/m3_repo_gdc_expression_matrix_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module3/m3_repo_gdc_expression_matrix_paths_status.flag"))
