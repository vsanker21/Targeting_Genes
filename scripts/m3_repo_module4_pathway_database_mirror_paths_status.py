#!/usr/bin/env python3
"""
Outline M3: presence of Module 4 m4_pathway_database_mirror outline artifacts under results/module4.

Config: config/m3_repo_module4_pathway_database_mirror_outline_inputs.yaml — path_checks block
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m3_repo_module4_pathway_database_mirror_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m3_repo_module4_pathway_database_mirror_path_checks") or {}
    if not block.get("enabled", True):
        print("m3_repo_module4_pathway_database_mirror_path_checks disabled")
        return 0
    ...
    note = (
        "M3 manifest slice for m4_pathway_database_mirror_paths_status and integration_stub under results/module4; "
        "data_root KEGG/Reactome trees remain in m4_pathway_database_mirror_paths_status.json groups."
    )
    out_doc = {
    ...
    d_out = "results/module3/m3_repo_module4_pathway_database_mirror_paths_status.json"
    d_flag = "results/module3/m3_repo_module4_pathway_database_mirror_paths_status.flag"
