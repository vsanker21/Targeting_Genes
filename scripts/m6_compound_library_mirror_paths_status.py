#!/usr/bin/env python3
"""
Outline M6 optional compound or ligand library mirror path presence under data_root.

Config: config/m6_compound_library_mirror_outline_inputs.yaml (m6_compound_library_mirror_path_checks).
"""
...
def main() -> int:
    rr = repo_root()
    dr = data_root()
    cfg_path = rr / "config" / "m6_compound_library_mirror_outline_inputs.yaml"
    if not cfg_path.is_file():
        print(f"Missing {cfg_path}", file=sys.stderr)
        return 1
    doc = yaml.safe_load(cfg_path.read_text(encoding="utf-8-sig"))
    block = doc.get("m6_compound_library_mirror_path_checks") or {}
    if not block.get("enabled", True):
        print("m6_compound_library_mirror_path_checks disabled")
        return 0
    ...
    out_doc = {
        ...
        "outline_module": 6,
        "artifact_kind": "m6_compound_library_mirror_paths_status",
        "groups": groups_out,
        "note": "Presence only; no RDKit, Open Babel, or docking prep jobs from this JSON.",
    }
    out_rel = str(block.get("output_json", "results/module6/m6_compound_library_mirror_paths_status.json"))
    flag_rel = str(block.get("done_flag", "results/module6/m6_compound_library_mirror_paths_status.flag"))
