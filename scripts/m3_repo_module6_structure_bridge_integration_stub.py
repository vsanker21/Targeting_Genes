#!/usr/bin/env python3
"""
Outline M3: Module 6 structure bridge path checklist vs bulk DEA repo stubs and structure_admet_integration_stub.

Config: config/m3_repo_module6_structure_bridge_outline_inputs.yaml — m3_repo_module6_structure_bridge_integration_stub
"""
...
def load_cfg() -> dict[str, Any]:
    p = repo_root() / "config" / "m3_repo_module6_structure_bridge_outline_inputs.yaml"
...
def main() -> int:
    rr = repo_root()
    doc = load_cfg()
    block = doc.get("m3_repo_module6_structure_bridge_integration_stub") or {}
    if not block.get("enabled", True):
        print("m3_repo_module6_structure_bridge_integration_stub disabled")
        return 0

    paths_doc = _read_json(rr, "results/module3/m3_repo_module6_structure_bridge_paths_status.json") or {}
    ...
    if not paths_doc:
        blockers.append(
            "Missing results/module3/m3_repo_module6_structure_bridge_paths_status.json "
            "(run m3_repo_module6_structure_bridge_paths_status)"
        )

    g = _group(paths_doc, "module6_structure_druggability_bridge_repo_outputs")
    n_ok = _n_ok(g)
    ...
    parent_signal = any(t and t != "D" for t in (wel_t, r3_t, strat_t, adm_t))
    ...
    payload = {
        ...
        "outline_module": 3,
        "artifact_kind": "m3_repo_module6_structure_bridge_integration_stub",
        "note": "Cross-module M3 manifest slice for M6 bridge TSVs and ADMET gap stub; no docking or structure prediction in-repo.",
        "paths_status_echo": {
            "module6_structure_druggability_bridge_repo_outputs": {"n_existing": n_ok, "n_checks": n_chk},
        ...
        "tier_legend": {
            "B": "All seventeen bridge and structure_admet stub paths present.",
            "C": "At least nine paths present, or a parent DEA / stratified / ADMET stub shows partial readiness.",
            "D": "Fewer than nine paths and no parent stub signal.",
        },
        "checklist": checklist,
        "blockers": blockers,
        "recommended_next_steps": [
            "Run export_module6_structure_druggability_bridge after GTS stub outline-driver TSVs and HGNC exist, then m6_structure_admet_integration_stub.",
            "Use module6_export_manifest for tooling paths alongside this M3 slice.",
        ],
    }

    out_rel = str(
        block.get("output_json", "results/module3/m3_repo_module6_structure_bridge_integration_stub.json")
    )
    flag_rel = str(
        block.get("done_flag", "results/module3/m3_repo_module6_structure_bridge_integration_stub.flag")
    )
